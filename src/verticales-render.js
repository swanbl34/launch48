/* ═══════════════════════════════════════════════════════════════════════════
   Gabarits des pages verticales — /offres/ et les 8 pages par secteur

   Module PUR : aucune référence à `document`, `window` ou au DOM. C'est la
   condition pour qu'il tourne aussi sous Node, au moment du build.

   POURQUOI CETTE SÉPARATION
   Ces 9 pages étaient entièrement construites par JavaScript à l'exécution :
   le HTML servi ne contenait qu'un `<div id="app"></div>` vide, titre `<h1>`
   compris. Écran blanc jusqu'à l'exécution du bundle, et rien du tout pour qui
   n'exécute pas JavaScript — sur des pages d'atterrissage SEO.

   Désormais `vite-plugin-prerender.js` appelle `renderPage()` pendant le build
   et écrit le HTML dans chaque fichier. Les mêmes fonctions restent utilisables
   côté navigateur, ce qui garde `offers-data.js` comme unique source de vérité :
   pas de gabarit dupliqué, donc pas de divergence possible entre ce que voit le
   robot et ce que voit le visiteur.

   RÈGLE À TENIR : ne rien introduire ici qui touche au navigateur. Le build
   casserait — c'est le comportement voulu, mieux vaut une erreur de build
   qu'une page qui redevient silencieusement vide.
   ═══════════════════════════════════════════════════════════════════════════ */
import { CONTACT, OFFERS, VERTICAL_DETAILS } from './offers-data.js';
...
/**
 * Rend une page complète à partir du type déclaré sur le `<body>`.
 *
 * Renvoie le HTML **et** les métadonnées : titre et description étaient
 * jusqu'ici posés par JavaScript après chargement, donc absents du document
 * servi. Ils sont maintenant écrits dans le fichier au build.
 *
 * @param {'home'|'offers'|'vertical'} pageType
 * @param {string} offerSlug  slug de l'offre, pour pageType === 'vertical'
 * @returns {{ title: string, description: string, html: string }}
 */
export const renderPage = (pageType, offerSlug = '') => {
  if (pageType === 'offers') return renderOffersPage();
  if (pageType === 'vertical') return renderVerticalPage(offerSlug);
  return renderHome();
};
